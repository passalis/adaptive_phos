...
class LOB_WF(Dataset):
    ...
    def __init__(self, h5_path='../dataset/lob.h5', split=1,
...
def get_wf_lob_loaders(h5_path='../dataset/lob.h5', window=50,
                       split=0, horizon=0, batch_size=128, class_resample=False, normalization='std', shift=None):
    """
    ...
    test_dataset = LOB_WF(h5_path=h5_path, split=split, train=False, n_window=window, normalization=normalization,
                          epsilon=1e-15, horizon=horizon, shift=shift)

    if class_resample:
        from lob_utils.sampler import ImbalancedDatasetSampler
        train_loader = DataLoader(train_dataset, batch_size=batch_size, num_workers=1, sampler=ImbalancedDatasetSampler(train_dataset))
    else:
